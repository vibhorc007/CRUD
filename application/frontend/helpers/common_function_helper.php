...
function upload_image($base64image, $type = 'jpg', $base = 'docs/') {
    $img = '';
    switch ($type) {
        case 'png':
            $filename = uniqid() . mt_rand(0, 99) . time() . '.png';
            $img = str_replace('data:image/png;base64,', '', $base64image);
            break;
        case 'jpg':
            $filename = uniqid() . mt_rand(0, 99) . time() . '.jpg';
            $img = str_replace('data:image/jpeg;base64,', '', $base64image);
            break;
        case 'jpeg':
            $filename = uniqid() . mt_rand(0, 99) . time() . '.jpeg';
            $img = str_replace('data:image/jpeg;base64,', '', $base64image);
            break;

        default :
            $filename = uniqid() . mt_rand(0, 99) . time() . '.' . $type;
            $img = $base64image;
    }

    $img = str_replace(' ', '+', $img);
    $data = base64_decode($img);
    $file = FCPATH . $base . $filename;
    $success = file_put_contents($file, $data);
    $file_information = get_file_info($file);
    $file_information['mime'] = get_mime_by_extension($file);
    $file_information['url'] = site_url($base . $filename);
    return $success ? $file_information : FALSE;
}
...
function gen_code($id = 0, $len = 4) {
    ...
    if ($id)
        $res .= dechex(intval($id));

    return strtoupper($res);
}

function genToken($len = 32, $id='') {
    $token = substr(md5(uniqid(rand(), true)), 0, $len);
    return $token.'-'.time().$id;
}

function user_unique_code($base = 'L', $id = 0, $len = 4, $new = true) {
    $ci = & get_instance();
    $sql = "SELECT concat(
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand($id)*4294967296))*36+1, 1),
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand(@seed)*4294967296))*36+1, 1),
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand(@seed)*4294967296))*36+1, 1),
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand(@seed)*4294967296))*36+1, 1),
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand(@seed)*4294967296))*36+1, 1),
              substring('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rand(@seed:=floor(rand(@seed)*4294967296))*36+1, 1),
    ...
    if ($new) {
        ...
        $r_code = $base . $result['code'];
    } else {
        $r_code = $base . dechex(intval($id)) . genToken($len);
    }


    $ci->db->select('unique_id');
    $ci->db->from('user');
    $ci->db->where('unique_id', $r_code);

    $q = $ci->db->get();

    if ($q->num_rows > 0) {
        return unique_code($base, $id, $len + 1, FALSE);
    }

    return $r_code;
}

function email($to, $subject = '', $message = '', $from = 'no-reply@dealert.com', $name = 'DEALert') {
    $ci = & get_instance();
    ...
    $ci->email->to($to);
    $ci->email->subject($subject . ' | DEALert');
...
function pagination($data) {
    ...
    $default = array(
        ...
        'cur_tag_close' => '</a></li>',
        'uri_segment' => $data['uri_segment']
    );
    $ci->pagination->initialize(array_merge($default, $data));
