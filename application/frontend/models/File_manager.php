...
class file_manager extends CI_Model {
    ...
    public function songs() {
       ...
       // $this->db->where("(file_type = 'video' or file_type = 'audio')");
    ...
    public function trash_AV($image_id) {
        ...
        $s = $query->row_array();
        if ($s) {
            unlink(FCPATH . 'songs/' . $s['file_name']); //here delete image from folder
        }
...
//            print_r($data);die;
    ...
    public function insert_AVI($data = array()) {
    ...
    public function delete_pic($user_id) {
        ...
        $this->db->delete('image');
        if ($result) {
            unlink(FCPATH . 'images/' . $result['file_name']); //here delete image from folder
        }
        $data['file_name'] = '';
        $this->db->where('user_id', $user_id);
