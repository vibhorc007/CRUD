                            ...
                            <div class="form-group">
                                ...
                                <div class=" row">
                                    <div class="recent-work-wrap col-sm-2">
                                        ...
                                        <img alt="" id="image_upload_preview" src="<?php echo str_replace('admin/','', base_url('images/glyphicons_user.png')); ?>" class="img img-responsive img thumbnail "height="200px" width="232px">
                                        <?php } else { ?>
                                        <img alt="" id="image_upload_preview" src="<?php echo str_replace('admin','',base_url('images') . '/' . $result['file_name']); ?>" class="img img-responsive img thumbnail "height="200px" width="232px">
                                        <?php }?>
                                        <div class="overlay  col-sm-1">
                                            <div class="recent-work-inner text-center" style="margin-top: 73px; margin-right: 44px;">
                                                ...
                                                <button type="button" onclick="myfunction(<?php echo $result['user_id']; ?>)" id="logo_delete" data-toggle="modal" data-target="#myModal2" class="btn btn-danger  hvr-grow-shadow"><i class="fa fa-trash-o"></i></button>
...
<div <?php echo (isset($upload_error) && $upload_error != '') ? 'id="upload_logo" class="modal fade in" aria-hidden="true" role="dialog" tabindex="-1" style="display: block;" ' : 'class="modal fade" id="upload_logo" tabindex="-1" role="dialog" aria-hidden="true" ' ?> >
    <div class="modal-dialog">
        <div class="modal-content" style="margin-top: 130px;">
            ...
            <div class="modal-body">
                ...
                <?php } ?>
                <!-- Custom Tabs -->
                <form action="<?php echo site_url('media/imageUpload' . '/' . $result['user_id']); ?>" method="post" enctype="multipart/form-data">
 ...
 <!--this is for image which we have upload for select image--><label for="<?php echo $image['image_id']; ?>"><input type="checkbox" name="image" value="<?php echo $image['image_id']; ?>" id="<?php echo $image['image_id']; ?>" <?php echo ($image['status'] == 1)?'checked':''?> /> <img  alt="<?php echo $this->session->username; ?>" src="<?php echo base_url() . 'images/' . $image['file_name']; ?>"></label>
                                                    ...
                                                    <input type="hidden" name="current_image" value="<?php  echo $image['image_id'];?>"/><!-- this is use for which pic is set on profile pic that will be hidden -->
                                                    <?php } ?>
                                                </li>
                                            <?php } ?>
                                        </ul><!-- /.users-list -->
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#logo_delete').click(function () {
            ...
            data += "<form action = '<?php echo site_url('media/delete_photo' . '/' . $result['user_id']); ?>'>";
